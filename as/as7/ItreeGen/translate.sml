...
fun subscript arrayStart indexExp =
  let val start = T.TEMP (Temp.newtemp()) and index = T.TEMP(Temp.newtemp())
      val offset = T.BINOP (T.MUL, index, (T.CONST F.wordSize))
   in Ex (T.ESEQ (seq [T.MOVE (start, unEx arrayStart),
                       T.MOVE (index, unEx indexExp),
                       T.EXP (runtimeCall "checkArrayBounds" [start, index])],
                  mem start offset))
  end
